...
class BasicReader(object):
    ...
    def read(self, data_path=data_path):
        ...
        # ent_testing.to_csv(data_path + "ent_testing1.csv")

        index(ent_mapping, e_map, None, is_kb=False)
        index(ent_testing, e_map, None, is_kb=False)
        # ent_testing.to_csv(data_path + "ent_testing2.csv")
...
# training procedure
def seq_train2(self, data, choices=None, epoch=None):
    opts = self._options
...
def handle_evaluation(i=0, last_mean_loss=0, kb_1to2=True, method='min', valid=True):
    ...
    if valid:
        ...
        # data = model._ent_testing.iloc[data_size // 10:]
        # data.to_csv(data_path + "test_data.csv")
        # data.to_csv(data_path + "test_input.csv")
        data = pd.read_csv(data_path + "test_input.csv")
    ...
    if valid == False:
        i = 0
        probs2 = probs
        ent_testing1 = pd.read_csv(data_path + "ent_testing1.csv")
        ent_testing2 = pd.read_csv(data_path + "ent_testing2.csv")
        for index, row in data.iterrows():
            # max_index = probs2[i].index(max(probs2[i]))
            max_ten_index = heapq.nlargest(10, range(len(probs2[i])), probs2[i].take)
            kb1_id = row['kb_1']
            kb2_id = row['kb_2']
            try:
                ent_testing_index = \
                    ent_testing2.loc[(ent_testing2['kb_2'] == kb2_id) & (ent_testing2['kb_1'] == kb1_id)].index[0]
                ent_testing_val = ent_testing1.loc[ent_testing_index]

                # output top10 of predict
                ent_testing_vals = []
                for max_index in max_ten_index:
                    ent_index = ent_testing2.loc[(ent_testing2['kb_1'] == max_index)]
                    if len(ent_index) > 0:
                        ent_testing_vals.append(ent_testing1.loc[ent_index.index[0]]['kb_1'])
                    else:
                        ent_index = ent_testing2.loc[(ent_testing2['kb_2'] == max_index)]
                        ent_testing_vals.append(ent_testing1.loc[ent_index.index[0]]['kb_2'])

                print('\ninput: %s \ntruth: %s \npredict: ' % (ent_testing_val['kb_1'], ent_testing_val['kb_2']))
                print(ent_testing_vals)
            except:
                print(kb1_id,kb2_id)
            i += 1
...
model_path = "../save/model.ckpt-10"
saver = tf.train.Saver()
print('----loading model----')
saver.restore(sess, model_path)
print('----model loaded----')
i = 0
last_mean_loss = seq_train2(model, train_data)
